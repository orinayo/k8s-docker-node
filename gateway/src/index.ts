...
function setupHandlers(app: Application) {
	...
	app.get('/video', (req: express.Request, res: express.Response) => {
		...
		http.request(
			...
			(response) => {
				...
				response.on('error', (err) => {
					console.error(
						`Failed to get details for video ${videoId}.`
					);
					console.error(err || `Status code: ${response.statusCode}`);
					res.sendStatus(500);
				});
	...
	app.get('/upload', (_: express.Request, res: express.Response) => {
		res.render('upload-video', {});
	});

	// Web page to show the users viewing history.
	app.get('/history', (_: express.Request, res: express.Response) => {
		http.request(
